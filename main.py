...
async def inference(request: Request) -> StreamingResponse:
    ...
    tts_speech, _ = audio_decoder.token2wav(tts_token, uuid=this_uuid, prompt_token=flow_prompt_speech_token.to(device),
                                            prompt_feat=prompt_speech_feat.to(device),finalize=is_finalize)
    # 将音频数据转换为字节
    result = io.BytesIO()
